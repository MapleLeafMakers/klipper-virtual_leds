...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/virtual_leds.py" "${KLIPPER_PATH}/klippy/extras/virtual_leds.py"
    echo "[OK]"
}
...
# Add updater for virtual_leds to moonraker.conf
add_updater()
{
    echo -e -n "Adding update manager to moonraker.conf... "

    update_section=$(grep -c '\[update_manager virtual_leds\]' ${MOONRAKER_CONFIG_DIR}/moonraker.conf || true)
    if [ "${update_section}" -eq 0 ]; then
        ...
        echo -e "[update_manager virtual_leds] already exists in moonraker.conf [SKIPPED]"
    fi
}
...
uninstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/virtual_leds.py" ]; then
        echo -n "Uninstalling... "
        rm -f "${KLIPPER_PATH}/klippy/extras/virtual_leds.py"
        echo "[OK]"
        echo "You can now remove the [update_manager virtual_leds] section in your moonraker.conf and delete this directory. Also remove all virtual_leds configurations from your Klipper configuration."
    else
        echo "virtual_leds.py not found in \"${KLIPPER_PATH}/klippy/extras/\". Is it installed?"
        echo "[FAILED]"
    fi
}
